...
extension FLLTest4ViewController {
    ...
    func createFlowLayout1(rootLayout: TGLinearLayout) {
        ...
        headerImageView.tg.top.equal(20)
        headerImageView.tg.bottom.equal(20)
        ...
        userNameTextField.tg.width.equal(.fill) //这里表示宽度用来占用流式布局每行的剩余宽度，这样就不需要明确的设置宽度了。
        userNameTextField.tg.height.equal(44)
        userNameTextField.tg.leading.equal(20)  //文本输入框距的左边间距为20
        ...
        passwordLabel.tg.width.min(userNameLabel.tg.width) //注意这里，因为"password"的长度要小于"User name",所以我们这里设定passwordLabel的最小宽度要和userNameLabel相等。这样目的是为了让后面的输入框具有左对齐的效果。
        passwordLabel.tg.top.equal(20)
        passwordLabel.sizeToFit()
        flowLayout.addSubview(passwordLabel)

        let passwordTextField = UITextField()
        passwordTextField.borderStyle = .roundedRect
        passwordTextField.tg.width.equal(.fill)  //表示宽度用来占用流式布局每行的剩余宽度，这样就不需要明确的设置宽度了。
        passwordTextField.tg.height.equal(44)
        passwordTextField.tg.top.equal(20)  //距离上行的顶部间距为20,注意这里要和passwordLabel设置的顶部间距一致，否则可能导致无法居中对齐.
        passwordTextField.tg.leading.equal(20)
        flowLayout.addSubview(passwordTextField)

        //the fourth line: forgot password.
        let placeHolderView2 = UIView()
        placeHolderView2.tg.width.equal(.fill) //因为流式布局这里面每行两列，所以这里建立一个宽高为0的占位视图。我们可以在流式布局中通过使用占位视图来充满行的数量。
        ...
        submitButton.tg.top.equal(20)
        submitButton.tg.height.equal(44)
        submitButton.tg.width.equal(flowLayout.tg.width, increment: -40)  //宽度等于父视图的宽度再减去40。
        //你也可以设置如下：
        //submitButton.tg.width.equal(100%, increment: -40)
        //或者
        //submitButton.tg.width.equal(.fill, increment: -40)
    ...
    func createFlowLayout2(rootLayout: TGLinearLayout) {
        ...
        headerImageView.tg.width.equal(80)
        headerImageView.tg.height.equal(80)
        headerImageView.layer.cornerRadius = 40
        headerImageView.layer.masksToBounds = true
        flowLayout.addSubview(headerImageView)

        let lineView = UIView()
        lineView.tg.width.equal(2)
        lineView.tg.height.equal(.fill)  //高度占用剩余空间
        ...
        for i in 0 ..< 9 {
            ...
            imageView.tg.leading.equal(10)
            imageView.tg.trailing.equal(10)
    ...
    func createFlowLayout3(rootLayout: TGLinearLayout) {
        ...
        v4.tg.width.equal((100/3.0)%, increment: -20)  //因为要均分为3部分，而我们设置了水平间距为10.所以我们这里要减去20。也就是减去2个间隔。
        v4.tg.height.equal(50)
        flowLayout.addSubview(v4)

        let v5 = UIView()
        v5.backgroundColor = CFTool.color(6)
        v5.tg.width.equal((100/2.0)%, increment: -10) //因为剩下的要均分为2部分，而我们设置了水平间距为10.所以我们这里要减去10。也就是减去1个间隔。
        v5.tg.height.equal(50)
        flowLayout.addSubview(v5)

        let v6 = UIView()
        v6.backgroundColor = CFTool.color(7)
        v6.tg.width.equal(100%)  //最后一个占用剩余的所有空间。这里没有间距了，所以不需要再减。
        v6.tg.height.equal(50)
    ...
    func createFlowLayout4(rootLayout: TGLinearLayout) {
        ...
        titleLabel.tg.width.equal(.fill)   //对于指定数量的流式布局来说这个weight的是剩余的占比。
        ...
        priceLabel.tg.width.equal(.wrap)
        priceLabel.tg.height.equal(30)
        ...
        buyButton.tg.width.equal(.wrap)
        buyButton.tg.height.equal(30)
        buyButton.tg.leading.equal(20)
