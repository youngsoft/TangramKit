...
class LLTest1ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        rootLayout.tg.insetsPaddingFromSafeArea(value: .all)
        //您可以在这里将值改变为UIRectEdge的其他类型然后试试运行的效果。并且在运行时切换横竖屏看看效果
        ...
        rootLayout.tg.insetLandscapeFringePadding(value: false)   //您可以在横屏下将这个属性设置为true后，然后尝试一下进行左右旋转后查看运行的效果。
        ...
        vertTitleLabel.tg.top.equal(self.topLayoutGuide, offset: 10)  //顶部距离前面的视图10
       // vertTitleLabel.tg.top.equal(10)  //您可以注释上面，解开这句看看运行效果
        rootLayout.addSubview(vertTitleLabel)

        let vertLayout = createVertSubviewLayout()
        vertLayout.tg.size(width: .fill, height: .wrap) //宽度和父视图保持一致，高度由子视图决定
        //您也可以使用如下方法来分别设置:
        // vertLayout.tg.width.equal(.fill)
        // vertLayout.tg.height.equal(.wrap)
        //您可以用如下的便捷方式来进行设置。~= 是equal方法的运算符赋值方式。
        //vertLayout.tg.width ~= .fill
        //vertLayout.tg.height ~= .wrap
        ...
        horzTitleLabel.tg.top.equal(10)  //顶部距离前面的视图10
        rootLayout.addSubview(horzTitleLabel)

        let horzLayout = createHorzSubviewLayout()
        horzLayout.tg.width.equal(.fill)    //宽度填充父视图宽度。
        horzLayout.tg.height.equal(.fill)   //高度填充父视图的剩余高度
        //您可以用如下方法分别设置
        //horzLayout.tg.size(width:.fill, height: .fill) //宽度由子视图决定，高度填充父视图的剩余高度
    ...
    @objc func RTLAction() {
        TGBaseLayout.tg.updateRTL(true, inWindow: self.view.window!)
    }

    @objc func LTRAction() {
        TGBaseLayout.tg.updateRTL(false, inWindow: self.view.window!)
    }
...
extension LLTest1ViewController {
    ...
    func createVertSubviewLayout() -> TGLinearLayout {
        ...
        /*
         ...
         2.tg.leading, tg_trailing的意义是子视图距离父视图的左右边距。
         3.tg.top, tg_bottom的意义是子视图和兄弟视图之间的上下间距。
         4.如果同时设置了tg_leading,tg_trailing则除了能确定左右边距，还能确定自身的宽度。
         5.如果同时设置了tg_top,tg_bottom则只能确定和其他兄弟视图之间的上下间距，但不能确定自身的高度。
         6.tg.centerX表示子视图的水平中心点在父视图的水平中心点上的偏移。
         7.tg.centerY的设置没有意义。
         */
        ...
        //您也可以用如下方式分别设置:
        // v1.tg.top.equal(10)        //上边边距10
        // v1.tg.leading.equal(10)       //左边边距10
        // v1.tg.width.equal(200)     //宽度200
        // v1.tg.height.equal(35)     //高度35
        //您也可以采用运算符的方式进行设置：
        // v1.tg.top ~= 10
        // v1.tg.leading ~= 10
        // v1.tg.width ~= 200
        ...
        v2.tg.top.equal(10)
        v2.tg.centerX.equal(0)   //水平居中的偏移,如果不等于0则会产生居中偏移
        v2.tg.width.equal(200)
        v2.tg.height.equal(35)   //等价于 v2.tg.size(width:200, height:35)
        vertLayout.addSubview(v2)

        let v3 = self.createLabel(NSLocalizedString("right margin", comment: ""), color: CFTool.color(7))
        v3.tg.top.equal(10)
        v3.tg.trailing.equal(10)  //右边边距10,因为这里只设置了右边边距，所以垂直线性布局会将子视图进行右对齐。
        ...
        v4.tg.top.equal(10)
        v4.tg.bottom.equal(10) // 注意这里虽然设置了上下的间距，但是对于垂直线性布局来说，同时设置上下间距并不能决定子视图的高度，只是表明子视图离兄弟视图的距离而已
        v4.tg.leading.equal(10)
        v4.tg.trailing.equal(10)  //上面两行代码将左右边距设置为10。对于垂直线性布局来说如果子视图同时设置了左右边距则宽度会自动算出，因此不需要设置tg_width的值了
        v4.tg.height.equal(35)  //这里仍然要设置子视图的高度。
    ...
    func createHorzSubviewLayout() -> TGLinearLayout {
        ...
        /*
         ...
         2.tg.top, tg_bottom的意义是子视图距离父视图的上下边距。
         3.tg.leading, tg_trailing的意义是子视图和兄弟视图之间的左右间距。
         4.如果同时设置了tg_top,tg_bottom则除了能确定上下边距，还能确定自身的高度。
         5.如果同时设置了tg_leading,tg_trailing则只能确定和其他兄弟视图之间的左右间距，但不能确定自身的宽度。
         6.tg.centerY表示子视图的垂直中心点在父视图的垂直中心点上的偏移。
         7.tg.centerX的设置没有意义。
         */
        ...
        v1.tg.height.equal(60)
        horzLayout.addSubview(v1)

        let v2 = self.createLabel(NSLocalizedString("vert center", comment: ""), color: CFTool.color(6))
        v2.tg.leading.equal(10)
        v2.tg.centerY.equal(0)   //垂直居中，如果不等于0则会产生居中偏移
        v2.tg.width.equal(60)
        v2.tg.height.equal(60)   //设置布局尺寸
        horzLayout.addSubview(v2)

        let v3 = self.createLabel(NSLocalizedString("bottom margin", comment: ""), color: CFTool.color(7))
        v3.tg.bottom.equal(10)
        v3.tg.leading.equal(10)
        v3.tg.trailing.equal(5)  //对于水平线性布局来说，同时设置左右间距并不能决定子视图的宽度，因此需要明确的设定宽度。
        v3.tg.width.equal(60)
        v3.tg.height.equal(60)   //设置布局尺寸
        horzLayout.addSubview(v3)

        let v4 = self.createLabel(NSLocalizedString("top bottom", comment: ""), color: CFTool.color(8))
        v4.tg.leading.equal(10)
        v4.tg.trailing.equal(10)
        v4.tg.top.equal(10)
        v4.tg.bottom.equal(10) //上面两行代码将上下边距设置为10,对于水平线性布局来说如果子视图同时设置了上下边距则高度会自动算出,因此不需要设置tg_height的值了。
