...
class TLTest1ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        _ = tableLayout.tg.addRow(size:30,colSize:70)
        
        var  colView = self.createLabel("Cell00", backgroundColor: CFTool.color(1))
        colView.tg.leading.equal(10)
        colView.tg.trailing.equal(40) //可以使用TGLayoutPos对象来调整间距。
        colView.tg.top.equal(5)
        colView.tg.bottom.equal(5)
        tableLayout.tg.addColumn(colView, inRow:0)
        
        colView = self.createLabel("Cell01", backgroundColor: CFTool.color(2))
        colView.tg.leading.equal(20)
        tableLayout.tg.addColumn(colView, inRow:0)
        ...
        //第二行固定高度为40，每列的宽度都相等。
        _ = tableLayout.tg.addRow(size:40,colSize:TGLayoutSize.average)
        
        colView = self.createLabel("Cell10", backgroundColor: CFTool.color(1))
        tableLayout.tg.addColumn(colView, inRow:1)
        ...
        _ = tableLayout.tg.addRow(size:30,colSize:TGLayoutSize.wrap)
        
        colView = self.createLabel("Cell20", backgroundColor: CFTool.color(1))
        colView.tg.width.equal(100)
        tableLayout.tg.addColumn(colView, inRow:2)
        
        colView = self.createLabel("Cell21", backgroundColor: CFTool.color(2))
        colView.tg.width.equal(200)
        tableLayout.tg.addColumn(colView, inRow:2)
        
        //第四行固定高度为30，子视图的宽度自己设置，但是每行的宽度和父视图保持一致。
        _ = tableLayout.tg.addRow(size:30,colSize:TGLayoutSize.fill)

        colView = self.createLabel("Cell30", backgroundColor: CFTool.color(1))
        colView.tg.width.equal(80)
        ...
        tableLayout.tg.addColumn(colView, inRow:4)
        ...
        _ = tableLayout.tg.addRow(size:30, colCount:4)
        
        colView = self.createLabel("Cell50", backgroundColor: CFTool.color(1))
        tableLayout.tg.addColumn(colView, inRow:5)
        ...
        _ = tableLayout.tg.addRow(size:TGLayoutSize.wrap, colSize:TGLayoutSize.average)
        
        colView = self.createLabel("Cell60", backgroundColor: CFTool.color(1))
        colView.tg.height.equal(80)
        tableLayout.tg.addColumn(colView, inRow:6)
        
        colView = self.createLabel("Cell61", backgroundColor: CFTool.color(2))
        colView.tg.height.equal(120)
        tableLayout.tg.addColumn(colView, inRow:6)
        
        colView = self.createLabel("Cell62", backgroundColor: CFTool.color(3))
        colView.tg.height.equal(70)
        tableLayout.tg.addColumn(colView, inRow:6)
