...
class AllTest3ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        scrollView.tg.width.equal(.fill)
        scrollView.tg.height.equal(.fill) //scrollView的尺寸和frameLayout的尺寸一致，因为这里用了填充属性。
        ...
        button.tg.height.equal(50);
        button.tg.width.equal(.fill)
        button.tg.bottom.equal(0) //按钮定位在框架布局的底部并且宽度填充。
...
extension AllTest3ViewController
{
    ...
    func addHeaderLayout(_ contentLayout: TGLinearLayout) {
        ...
        headerImageView.tg.centerX.equal(0)
        headerImageView.tg.centerY.equal(0) //在父视图中居中。
        headerImageView.layer.cornerRadius = headerImageView.tg.estimatedFrame.height / 2
        ...
        headerNameLabel.tg.centerX.equal(0)
    ...
    func addVisibilityTestLayout(_ contentLayout: TGLinearLayout)
    {
        ...
        invisibleButton.tg.height.equal(.wrap)
        invisibleButton.tg.width.equal(.average)
        ...
        goneButton.tg.height.equal(.wrap)
        goneButton.tg.width.equal(.average)
    ...
    func addFlexedWidthLayout(_ contentLayout: TGLinearLayout)
    {
        ...
        leftLabel.tg.trailing.equal(50%).min(0) //右边浮动间距为0.5,最小为0
        leftLabel.tg.width.equal(.wrap)
        ...
        rightLabel.tg.leading.equal(50%).min(0) //左边浮动间距为0.5，最小为0
        rightLabel.tg.width.equal(.wrap)
    ...
    func addActiveLayout(_ contentLayout:TGLinearLayout)
    {
        ...
        testButton.tg.height.equal(50)
        testButton.tg.width.equal(.wrap,increment:20)
        testButton.tg.leading.equal(10).isActive = true  //左边边距是10，设置active为YES表示左边位置对象的设置是生效的。
        testButton.tg.trailing.equal(10).isActive = false  //右边边距是10，设置active为NO表示右边位置对象的设置是不生效的。
...
extension AllTest3ViewController
{
    ...
    @objc func handleShrinkSwitch(sender: UISwitch) {
        if sender.isOn {
            self.shrinkLayout.tg.height.equal(.wrap)
        }
        else {
            self.shrinkLayout.tg.height.equal(50)
        }
        
        self.shrinkLayout.tg.layoutAnimationWithDuration(0.3)
    }
    ...
    @objc func handleShowPopMenu(sender: TGBaseLayout) {
        ...
        menuLayout.tg.width.equal(rc.width - 20) //宽度是sender的宽度减20
        menuLayout.tg.height.equal(.wrap);
        menuLayout.tg.centerX.equal(0) //因为我们是把弹出菜单展示在self.view下，这时候self.view是一个框架布局。所以这里这是水平居中。
        menuLayout.tg.top.equal(rc.maxY + 5)//弹出菜单的顶部位置。
        ...
        let size = itemLayout.tg.sizeThatFits(CGSize(width:rc.width - 40, height:0))
        scrollView.tg.height.equal(size.height).min(50).max(155)
        ...
        self.frameLayout.tg.layoutAnimationWithDuration(0.3)
    }
    ...
    @objc func handleAddMe(sender: UIButton) {
        ...
        let size = self.popmenuItemLayout.tg.sizeThatFits()
        self.popmenuScrollView.tg.height.equal(size.height)
        //多个布局同时动画。
        self.popmenuItemLayout.tg.layoutAnimationWithDuration(0.3)
        self.popmenuLayout.tg.layoutAnimationWithDuration(0.3)
        self.popmenuContainerLayout.tg.layoutAnimationWithDuration(0.3)
    }
    ...
    @objc func handleClosePopmenu(sender: UIButton!) {
        ...
        self.popmenuLayout.tg.top.equal(self.frameLayout.frame.size.height)
    ...
    @objc func handleActiveTest(sender:UIButton!)
    {
        ...
        if (sender.tg.leading.isActive && sender.tg.trailing.isActive)
        {
            sender.tg.width.equal(.wrap, increment:20)  //恢复原来的宽度。
            sender.tg.leading.isActive = true
            sender.tg.trailing.isActive = false  //按钮将停靠在父布局的左边。
        }
        else if (sender.tg.leading.isActive)
        {
            sender.tg.leading.isActive = false
            sender.tg.trailing.isActive = true  //按钮将停靠在父布局的右边
        }
        else if (sender.tg.trailing.isActive)
        {
            sender.tg.leading.isActive = true
            sender.tg.trailing.isActive = true  //按钮的左右边距都生效，并且会拉伸按钮的宽度。
        }
        
        if let superLayout = sender.superview as? TGLinearLayout
        {
            superLayout.tg.layoutAnimationWithDuration(0.3)
        }
