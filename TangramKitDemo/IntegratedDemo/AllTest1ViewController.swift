...
class AllTest1ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        /*
         ...
         而如果某个布局视图的高度有可能是动态的高度，也就是用了tg.height为.wrap时，可以不用指定明确的指定高度，但要指定宽度。而且在布局视图添加到self.tableView.tableHeaderView 之前一定要记得调用：
