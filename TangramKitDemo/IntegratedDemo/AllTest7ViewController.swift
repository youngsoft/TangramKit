...
extension AllTest7ViewController
{
    ...
    func createDemo4(_ rootLayout: TGLinearLayout) {
        ...
        /*
         ...
         fixedLabel.tg.width.equal(40)
         fixedLabel.tg.height.equal(30)
        ...
        leftLabel.tg.width.equal(.wrap)//设置宽度等于自身内容的宽度
        ...
        rightLabel.tg.width.equal(.wrap)  //设置宽度等于自身内容的宽度
        rightLabel.tg.height.equal(.wrap)
...
//                tableLayout.tg.addRow(size:TGLayoutSize.wrap, colSize: TGLayoutSize.fill).tg.shrinkType = .average
    ...
    func createDemo7(_ rootLayout: TGLinearLayout) {
        ...
        for i in 0..<15 {
            ...
            label.tg.width.equal(CGFloat(arc4random() % 150 + 20)) //最宽170，最小20.
            label.tg.height.equal(30)
