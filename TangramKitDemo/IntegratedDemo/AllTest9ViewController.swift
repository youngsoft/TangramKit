...
class AllTest9ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        rootLayout.tg.gravity(value: TGGravity.Horizontal.fill) //里面所有子视图的宽度都填充为和父视图一样宽。
        ...
        actionLayout.tg.gravity(value: TGGravity.Horizontal.fill) //所有子视图水平填充，也就是所有子视图的宽度相等。
        ...
        contentLayout.tg.space(value: 10)
...
extension AllTest9ViewController
{
    ...
    {
        //还原位置的坐标变换，这也是默认值。
        self.contentLayout.tg.layoutTransform(value: .identity)
    ...
    @objc func handleTranslationTransform(_ sender:AnyObject?)
    {
        ...
        {
            self.contentLayout.tg.layoutTransform(value: CGAffineTransform(translationX: 100, y: 0))
        }
        else if self.contentLayout.tg.layoutTransform() == CGAffineTransform(translationX: 100, y: 0)
        {
            self.contentLayout.tg.layoutTransform(value: CGAffineTransform(translationX: 100, y: 100))
        }
        else
        {
            self.contentLayout.tg.layoutTransform(value: CGAffineTransform.identity)
        }
    ...
    @objc func handleScaleTransform(_ sender:AnyObject?)
    {
        ...
        self.contentLayout.tg.layoutTransform(value: CGAffineTransform(a:factor, b:0, c:0, d:factor, tx:(1 - factor) * (20 - size.width / 2.0), ty:(1 - factor) * (20 - size.height / 2))) //这里因为要让第一个子视图的位置保持不变，所以tx,ty参数需要进行特殊设置。
    ...
    @objc func handleHorzReflectionTransform(_ sender:AnyObject?)
    {
        ...
        self.contentLayout.tg.layoutTransform(value: CGAffineTransform(a:-1,b:0,c:0,d:1,tx:0,ty:0))
        self.contentLayout.tg.layoutAnimationWithDuration(0.3)
    }
