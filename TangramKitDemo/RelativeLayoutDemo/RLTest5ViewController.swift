...
class RLTest5ViewController: UIViewController {
    ...
    @objc func handleButtonSelect(_ button: UIButton) {
        ...
        //调整underLineView的位置。
        underLineView.tg.leading.equal(button.tg.leading)
        underLineView.tg.width.equal(button.tg.width)

        containerLayout.tg.layoutAnimationWithDuration(0.3)
    ...
    func createDemo1(_ rootLayout: UIView) {
        ...
        //根据内容得到高度和宽度
        leftButton.tg.leading.min(containerLayout.tg.leading)
        //左边最小边界是父视图左边偏移0
        leftButton.tg.trailing.max(containerLayout.tg.centerX)
        //右边最大的边界是父视图中心点偏移0
        ...
        rightButton.tg.leading.min(containerLayout.tg.centerX) //左边最小边界是父视图中心点偏移0
        rightButton.tg.trailing.max(containerLayout.tg.trailing)//右边最大边界是父视图右边偏移0
        ...
        underLineView.tag = 3
        underLineView.tg.height.equal(1)
        underLineView.tg.width.equal(leftButton.tg.width)
        underLineView.tg.leading.equal(leftButton.tg.leading)
        underLineView.tg.top.equal(leftButton.tg.bottom, offset: 6)
    ...
    func createDemo2(_ rootLayout: UIView) {
        ...
        for i in 0..<images.count {
            ...
            leftImageView.tg.top.equal(90 * i)
            ...
            flexedLabel.tg.height.equal(.wrap) //高度自动计算。
            flexedLabel.tg.leading.equal(leftImageView.tg.trailing, offset: 5) //左边等于leftImageView的右边
            flexedLabel.tg.top.equal(leftImageView.tg.top) //顶部和leftImageView相等。
            containerLayout.addSubview(flexedLabel)

            let  editImageView = UIImageView(image: UIImage(named: "edit")!)
            editImageView.tg.leading.equal(flexedLabel.tg.trailing)//这个图片总是跟随在flexedLabel的后面。
            editImageView.tg.top.equal(leftImageView.tg.top, offset: 4)
            ...
            rightLabel.tg.trailing.equal(containerLayout.tg.trailing) //右边等于父视图的右边，也就是现实在最右边。
            rightLabel.tg.top.equal(leftImageView.tg.top, offset: 4)
            containerLayout.addSubview(rightLabel)

            flexedLabel.tg.width.equal(.wrap) //宽度等于自身的宽度
            flexedLabel.tg.trailing.max(rightLabel.tg.leading, offset: editImageView.frame.width + 10)//右边的最大的边界就等于rightLabel的最左边再减去editImageView的尺寸外加上10,这里的10是视图之间的间距，为了让视图之间保持有足够的间距。这样当flexedLabel的宽度超过这个最大的右边界时，系统自动会缩小flexedLabel的宽度，以便来满足右边界的限制。 这个场景非常适合某个UITableViewCell里面的两个子视图之间有尺寸长度约束的情况。
        }
    }
    ...
    func createDemo3(_ rootLayout: UIView) {
        ...
        leftLabel.tg.width.equal(100)//宽度固定为100
        leftLabel.tg.height.equal(.wrap) //高度由子视图的内容确定，自动计算高度。
        leftLabel.tg.top.min(containerLayout.tg.top)//最小的顶部位置是父布局的顶部。
        leftLabel.tg.bottom.max(containerLayout.tg.bottom) //最大的底部位置是父布局的底部
        ...
        rightLabel.tg.trailing.equal(containerLayout.tg.trailing) //和父布局视图右对齐。
        rightLabel.tg.centerY.equal(leftLabel.tg.centerY) //和左边视图垂直居中对齐。
        rightLabel.tg.leading.min(leftLabel.tg.trailing, offset: 10) //右边视图的最小左边界是等于左边视图的右边偏移10，这样当右边视图的宽度超过这个最小边界时则会自动压缩视图的宽度。

        rightLabel.tg.width.equal(.wrap) //宽度等于自身的宽度。这个设置和上面的tg_leading.min方法配合使用实现子视图宽度的压
        rightLabel.tg.height.equal(.wrap).max(containerLayout.tg.height) //但是最大的高度等于父布局视图的高度(注意这里内部自动减去了padding的值)
        containerLayout.addSubview(rightLabel)
        //添加手势处理。
