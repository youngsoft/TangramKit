...
class RLTest4ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        v1.tg.width.equal(.fill)  //填充父视图的剩余宽度。
        v1.tg.height.equal(80)
        rootLayout.addSubview(v1)
        self.testView1 = v1
        
        
        let v2 = self.createLabel("", backgroundColor: CFTool.color(2))
        v2.tg.width.equal(100%)  //占用父视图宽度的100%
        v2.tg.height.equal(200);
        rootLayout.addSubview(v2)
        
        
        let v3 = self.createLabel("", backgroundColor: CFTool.color(3))
        v3.tg.leading.equal(0)
        v3.tg.trailing.equal(0)    //左右边距为0表示宽度和父视图相等。
        v3.tg.height.equal(800)
        v3.tg.top.equal(v2.tg.bottom)
        ...
        v4.tg.width.equal(rootLayout.tg.width)   //和父视图一样宽。
        v4.tg.height.equal(80)
        v4.tg.top.equal(v1.tg.bottom)
        rootLayout.addSubview(v4)
        self.testTopDockView = v4;
        
        v2.tg.top.equal(v4.tg.bottom)
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView)
    {
        ...
        if (scrollView.contentOffset.y > self.testView1.frame.maxY)
        {
            ...
            /*
             ...
             v4.tg.width.equal(rootLayout.tg.width);  //宽度和父视图相等。
             v4.tg.height.equal(80);              //高度等于80。
             v4.tg.top.equalTo(v1.tg.bottom);         //总是位于v1的下面。
             。。。。
             v2.tg.top.equalTo(v4.tg.bottom);         //v2则总是位于v4的下面。
