...
class FOLTest3ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        floatLayout.tg.intelligentBorderline(value: TGBorderline(color: UIColor.lightGray.withAlphaComponent(0.2)))
...
extension FOLTest3ViewController {
    ...
    func createPictureNewsItemLayout(_ dataModel: FOLTest3DataModel, tag: Int) -> TGBaseLayout {
        ...
        itemLayout.tg.highlightedBackgroundColor(value: .lightGray)
        itemLayout.tg.backgroundImage(value: UIImage(named: dataModel.imageName))  //将图片作为布局的背景图片
        itemLayout.tg.padding(value: UIEdgeInsets.init(top: 10, left: 10, bottom: 10, right: 10))
        itemLayout.tg.gravity(value: TGGravity.Vertical.bottom)  //将整个布局中的所有子视图垂直居底部。
    ...
    func createWholeWidthTextNewsItemLayout(_ dataModel: FOLTest3DataModel, tag: Int) -> TGBaseLayout {
        ...
        itemLayout.tg.padding(value: UIEdgeInsets.init(top: 10, left: 10, bottom: 10, right: 10))
        itemLayout.tg.highlightedBackgroundColor(value: .lightGray)
        itemLayout.tg.gravity(value: TGGravity.Vertical.center) //将整个布局中的所有子视图整体垂直居中。
        ...
        sourceLabel.tg.clearFloat(value: true) //换行
    ...
    func createHalfWidthTextNewsItemLayout(_ dataModel: FOLTest3DataModel, tag: Int) -> TGBaseLayout {
        ...
        itemLayout.tag = tag
        itemLayout.tg.setTarget(self, action: #selector(handleItemLayoutClick), for: .touchUpInside)
        itemLayout.tg.padding(value: UIEdgeInsets(top: 10, left: 10, bottom: 10, right: 10))
        itemLayout.tg.highlightedBackgroundColor(value: .lightGray)
        itemLayout.tg.gravity(value: TGGravity.Vertical.center)  ////将整个布局中的所有子视图整体垂直居中。
        ...
        sourceLabel.textColor = UIColor.lightGray
        sourceLabel.sizeToFit()
        sourceLabel.tg.clearFloat(value: true)
        sourceLabel.tg.top.equal(5)
        itemLayout.addSubview(sourceLabel)
